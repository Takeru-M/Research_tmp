...
- 問いとは別に、
  「なぜその点を検討する必要があるのか」という理由を
  明示的な一文として必ず含める
- その理由は、
  意思決定や評価の妥当性にどのような影響があり得るかが
  資料内容から分かる形で述べる
- 少なくとも1つの具体例を含める
- 結論や評価を誘導しない
...
- 内部構造や処理を想起させる語を使わない
- 問いとは別に、
  なぜ再検討する意味があるのかを説明する一文を
  明示的に含める
- 不自然に複数の話題を詰め込んだ文にならないようにする
...
- 示唆文の中に、
  「なぜ考える必要があるのか」を説明する独立した一文が
  明確に含まれているか
- 問いが不自然に複数の話題を詰め込んだ文になっていないか
...
2. 最新コメントが前提としている選択肢・仮定・評価軸を把握する  
3. highlightText がある場合は、その前提や判断の幅を再点検する  
4. 再検討の方向性を示す問いを 1 つ構成する  
...
- 再検討することにどのような意味があるのかという
  「考える価値がある理由」を、簡潔な一文として必ず含める
- 理由は、
  判断の妥当性、想定外の影響、利用上の支障などの
  いずれか一つに絞って述べる
- 再検討すべき観点や代替条件は 1〜2 個までに限定する
- 具体例を含める場合も 1 例までとし、
  条件の列挙や多段の説明は行わない
- 単なる要約・言い換えは禁止
- 直接的な解答・判断・評価は禁止
...
- 理由を説明する一文と問いが、
  過剰に分岐せず一つの話題としてまとまっていること
- 観点・代替条件の提示は 1〜2 個までに抑える
